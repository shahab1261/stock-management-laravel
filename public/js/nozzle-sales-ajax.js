document.addEventListener('DOMContentLoaded', function(){
    ...
    $(document).on('click', '#openNozzleSalesModalBtn', function(){
        ...
        $('#nozzles_div').html('');
        $('#notes').val('');
        var modal = new bootstrap.Modal(document.getElementById('nozzleSalesModal'));
        modal.show();
    });

    // calculate totals on changes
    $(document).on('change', '.closing_reading, .sale_rate, .textbox', function(){
    ...
    function getquantityandamount(){
        ...
        $(".closing_reading").each(function(){
            ...
            var closing_reading = parseFloat($('#closing_reading_'+nozzle_id).val()||0);
            var opening_reading = parseFloat($('#opening_reading_'+nozzle_id).val()||0);
        ...
        $('#quantity_total').html('<b>'+actual_quantity_total.toFixed(2)+'</b>');
        $('#amount_total').html('<b>'+grandtotalamount.toFixed(2)+' Rs </b>');
    }
    ...
    $(document).on('change', '#product_id', function(){
        ...
        $.post({
            url: window.nozzleProductNozzlesUrl || '/sales/nozzle/product-nozzles',
            ...
            success: function(resp){
                ...
                $('#current_rate').html('Rate: <b>Rs '+resp.current_sale+'</b>');
    ...
    $(document).on('submit', '#addSalesForm_pump', function(e){
        ...
        $.post({
            ...
            data: { _token: $('meta[name="csrf-token"]').attr('content'), product_id: $('#product_id').val(), sales_date: $('#sale_date').val() },
            success: function(pre){
                ...
                $(".closing_reading").each(function(){
                    ...
                    if($('#quantity_'+nozzle_id).val()==='0' && $('#testbox_'+nozzle_id).val()==='0'){
                    ...
                    requests.push($.post({ url: window.nozzleStoreUrl || '/sales/nozzle/store', data: payload }));
                });
                ...
                $.when.apply($, requests).done(function(){
                    ...
                    if (xhr && xhr.responseJSON && xhr.responseJSON.error === 'tank-limit-exceed') {
