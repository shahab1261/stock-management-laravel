document.addEventListener('DOMContentLoaded', function(){
    ...
    $(document).on('click', '#openLubricantSalesModalBtn', function(){
        resetLubricantForm();
        var modal = new bootstrap.Modal(document.getElementById('lubricantSalesModal'));
        modal.show();
    });
    ...
    $(document).on('change', '.sale_rate, .quantity', function(){ totals(); });
    ...
    function totals(){
        ...
        $(".opening_reading").each(function(){
            ...
            var opening_reading = parseFloat($('#opening_reading_'+product_id).val()||0);
            var sale_rate = parseFloat($('#rate_'+product_id).val()||0);
            if (closing_reading < opening_reading){
                Swal.fire({ icon:'info', title:'Alert', text:'Quantity can not be negative', confirmButtonColor:'#4154f1' });
                $('#lubricant_add_sales_btn').attr('disabled', true).text('Submit');
                return false;
            }
            ...
            if (actual_quantity < 0){
                Swal.fire({ icon:'info', title:'Alert', text:'Quantity can not be less than 0', confirmButtonColor:'#4154f1' });
                $('#lubricant_add_sales_btn').attr('disabled', true).text('Submit');
                return false;
            }
            if (opening_reading < actual_quantity){
                Swal.fire({ icon:'info', title:'Alert', text:'Insufficient stock', confirmButtonColor:'#4154f1' });
                $('#lubricant_add_sales_btn').attr('disabled', true).text('Submit');
                return false;
            }
        ...
        $('#lubricant_quantity_total').html('<b>'+actual_quantity_total.toFixed(2)+'</b>');
        $('#lubricant_amount_total').html('<b>'+grandtotalamount.toFixed(2)+' Rs </b>');
    }
    ...
    $(document).on('click', '.delete-sale-btn', function(){
        ...
        Swal.fire({
            ...
            $.ajax({
                ...
                method: 'POST',
                data: { _token: $('meta[name="csrf-token"]').attr('content'), sales_id: saleId },
                success: function(resp){
                    if (resp && resp.success) {
                        Swal.fire({ icon:'success', title:'Deleted', text: resp.message || 'Sale deleted', confirmButtonColor:'#4154f1' })
                            .then(()=>{ location.reload(); });
                    } else {
                        Swal.fire({ icon:'error', title:'Error', text: (resp && resp.message) || 'Failed to delete sale', confirmButtonColor:'#4154f1' });
                    }
                },
                error: function(){
                    Swal.fire({ icon:'error', title:'Error', text: 'Failed to delete sale', confirmButtonColor:'#4154f1' });
                }
            });
        });
    });
