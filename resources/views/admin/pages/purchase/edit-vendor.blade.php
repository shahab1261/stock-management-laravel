...
@section('title', 'Edit Purchase Vendor')
@section('description', 'Update vendor account for this purchase')
...
<div class="container-fluid py-4">
    <div class="row mb-4">
        <div class="col-12 text-center">
            ...
            <p class="text-muted mb-0">Update only the vendor account for this purchase</p>
        </div>
    </div>

    <form id="editVendorForm" action="{{ route('purchase.update-vendor', $purchase->id) }}" method="POST">
        @csrf
                        ...
                        <div class="input-group">
                            ...
                            <select class="form-select border-start-0 searchable-dropdown" name="vendor_id" id="vendor" required>
                                ...
                                @foreach ($suppliers as $supplier)
                                    <option value="{{ $supplier->id }}" data-type="1" {{ $purchase->vendor_type == 1 && $purchase->supplier_id == $supplier->id ? 'selected' : '' }}>{{ str_replace('&amp;', '&', $supplier->name) }} (Supplier)</option>
                                @endforeach
                                @foreach ($customers as $customer)
                                    <option value="{{ $customer->id }}" data-type="2" {{ $purchase->vendor_type == 2 && $purchase->supplier_id == $customer->id ? 'selected' : '' }}>{{ str_replace('&amp;', '&', $customer->name) }} (Customer)</option>
                                @endforeach
                                @foreach ($products as $product)
                                    <option value="{{ $product->id }}" data-type="3" {{ $purchase->vendor_type == 3 && $purchase->supplier_id == $product->id ? 'selected' : '' }}>{{ str_replace('&amp;', '&', $product->name) }} (Product)</option>
                                @endforeach
                                @foreach ($expenses as $expense)
                                    <option value="{{ $expense->id }}" data-type="4" {{ $purchase->vendor_type == 4 && $purchase->supplier_id == $expense->id ? 'selected' : '' }}>{{ str_replace('&amp;', '&', $expense->expense_name) }} (Expense)</option>
                                @endforeach
                                @foreach ($incomes as $income)
                                    <option value="{{ $income->id }}" data-type="5" {{ $purchase->vendor_type == 5 && $purchase->supplier_id == $income->id ? 'selected' : '' }}>{{ str_replace('&amp;', '&', $income->income_name) }} (Income)</option>
                                @endforeach
                                @foreach ($banks as $bank)
                                    <option value="{{ $bank->id }}" data-type="6" {{ $purchase->vendor_type == 6 && $purchase->supplier_id == $bank->id ? 'selected' : '' }}>{{ str_replace('&amp;', '&', $bank->name) }} (Bank)</option>
                                @endforeach
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}" data-type="9" {{ $purchase->vendor_type == 9 && $purchase->supplier_id == $employee->id ? 'selected' : '' }}>{{ str_replace('&amp;', '&', $employee->name) }} (Employee)</option>
                                @endforeach
                                <option value="7" data-type="7" {{ $purchase->vendor_type == 7 && $purchase->supplier_id == 7 ? 'selected' : '' }}>Cash</option>
                                <option value="8" data-type="8" {{ $purchase->vendor_type == 8 && $purchase->supplier_id == 8 ? 'selected' : '' }}>Mp</option>
                            </select>
                            <input type="hidden" name="vendor_data_type" id="vendor_data_type" value="{{ $purchase->vendor_type }}">
                        </div>
                    </div>
                </div>
            </div>
        ...
        if (form) {
            form.addEventListener('submit', async function(e){
                ...
                try {
                    ...
                    setTimeout(function(){
                        window.location.href = data.redirect || '{{ route('purchase.index') }}';
                    }, 900);
                } catch (err) {
